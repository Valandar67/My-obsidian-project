...
const CONFIG = {
    ...
    // Level radii (distance from center for each depth level) - Reduced for tighter spacing
    radii: [0, 160, 280, 380, 470, 560],

    // Node sizes - Reduced for more compact visual
    rootRadius: 40,
    nodeRadius: 32,
    ...
    // Timing
    longPressTime: 400,  // Reduced from 800ms for faster selection
...
// Draw stars on main canvas (called from render()) - Static, no animation
function drawStars() {
    ...
    // Draw stars - static, no twinkling
    stars.forEach(star => {
        ctx.beginPath();
...
// ==========================================
// IMPORTANCE RINGS (Fallen Order Style)
// Etched concentric rings matching reference image aesthetic
// ==========================================
function drawImportanceRing(x, y, nodeRadius, importance, isSelected) {
    if (importance === 0 && !isSelected) return;

    // Use seeded random for consistent procedural generation
    const seed = Math.abs(x * 1000 + y);

    // Helper function for drawing etched arc segments with refined appearance
    function drawEtchedRing(radius, lineWidth, alpha, segments, gapChance) {
        const segmentAngle = (Math.PI * 2) / segments;

        for (let i = 0; i < segments; i++) {
            const pseudoRandom = Math.sin(seed + i * 12.9898) * 43758.5453 % 1;
            const pseudoRandom2 = Math.sin(seed + i * 78.233 + 1) * 43758.5453 % 1;
            const pseudoRandom3 = Math.sin(seed + i * 45.164 + 2) * 43758.5453 % 1;

            // Skip segments for eroded/etched effect
            if (pseudoRandom < gapChance) continue;

            const startAngle = i * segmentAngle;
            const angleVariation = (pseudoRandom2 - 0.5) * segmentAngle * 0.2;
            const endAngle = startAngle + segmentAngle * 0.82 + angleVariation;

            // Minimal radius jitter for cleaner look
            const radiusJitter = (pseudoRandom3 - 0.5) * 0.8;
            const r = radius + radiusJitter;

            // Slight alpha variation for organic feel
            const alphaVariation = (pseudoRandom2 - 0.5) * 0.1;
            const finalAlpha = Math.max(0.2, alpha + alphaVariation);
    ...
    // TIER 1: Single thin etched line
    if (importance === 1) {
        drawEtchedRing(nodeRadius + 10, 1.2, 0.42, 14, 0.12);
    }
    // TIER 2: Double ring pattern
    else if (importance === 2) {
        drawEtchedRing(nodeRadius + 9, 1.4, 0.55, 16, 0.14);
        drawEtchedRing(nodeRadius + 15, 1.2, 0.45, 18, 0.16);
    }
    // TIER 3: Triple ring with connecting lines
    else if (importance === 3) {
        // Inner ring
        drawEtchedRing(nodeRadius + 8, 1.5, 0.62, 18, 0.12);
        // Middle ring
        drawEtchedRing(nodeRadius + 14, 1.6, 0.68, 20, 0.14);
        // Outer ring
        drawEtchedRing(nodeRadius + 20, 1.3, 0.55, 22, 0.16);

        // Radial connection lines (like the reference image)
        ctx.strokeStyle = 'rgba(255, 255, 255, 0.35)';
        ctx.lineWidth = 0.9;
        for (let i = 0; i < 8; i++) {
            const pseudoRandom = Math.sin(seed + i * 34.567) * 43758.5453 % 1;
            if (pseudoRandom < 0.38) continue; // Skip some lines

            const angle = (i / 8) * Math.PI * 2 + (pseudoRandom - 0.5) * 0.15;
            ctx.beginPath();
            ctx.moveTo(x + Math.cos(angle) * (nodeRadius + 8), y + Math.sin(angle) * (nodeRadius + 8));
            ctx.lineTo(x + Math.cos(angle) * (nodeRadius + 20), y + Math.sin(angle) * (nodeRadius + 20));
            ctx.stroke();
        }
    }
    // TIER 4 (Pinnacle): Complex multi-ring with glow
        ...
        ctx.beginPath();
        ctx.arc(x, y, nodeRadius + 30, 0, Math.PI * 2);
        ctx.fill();

        // Multiple concentric rings with refined spacing
        drawEtchedRing(nodeRadius + 7, 1.8, 0.72, 20, 0.1);
        drawEtchedRing(nodeRadius + 13, 1.6, 0.68, 22, 0.12);
        drawEtchedRing(nodeRadius + 19, 1.7, 0.65, 24, 0.14);
        drawEtchedRing(nodeRadius + 25, 1.4, 0.58, 26, 0.16);

        // Radial lines with slight angle variation
        ctx.strokeStyle = 'rgba(255, 255, 255, 0.45)';
        ctx.lineWidth = 1.1;
        for (let i = 0; i < 12; i++) {
            const pseudoRandom = Math.sin(seed + i * 45.678) * 43758.5453 % 1;
            if (pseudoRandom < 0.32) continue;

            const angle = (i / 12) * Math.PI * 2 + (pseudoRandom - 0.5) * 0.12;
            ctx.beginPath();
            ctx.moveTo(x + Math.cos(angle) * (nodeRadius + 7), y + Math.sin(angle) * (nodeRadius + 7));
            ctx.lineTo(x + Math.cos(angle) * (nodeRadius + 25), y + Math.sin(angle) * (nodeRadius + 25));
            ctx.stroke();
        }
    }
}
...
function drawNode(node) {
    ...
    // Draw importance ring first (behind the node)
    if (importance > 0 || isSelected) {
        ctx.save();
        ctx.shadowBlur = 0;
        drawImportanceRing(x, y, radius, importance, isSelected);
        ctx.restore();
    }
    ...
    drawIcon(x, y, radius * 0.55, node.icon || 'default', isSelected);
...
const svgIconCache = new Map(); // Cache for SVG icons to prevent reload
...
function drawIcon(x, y, size, iconName, isSelected = false) {
    ...
    if (isCustomImagePath(iconName)) {
        ...
        if (state === 'loaded' && customImageCache.has(iconName)) {
            ...
            // Draw error indicator (red X) with fallback
            ctx.save();
            ...
            // Start loading or show loading indicator
            if (state !== 'loading') {
                loadCustomImage(iconName, () => {});
            }
            // Draw loading placeholder (faint circle)
            ctx.save();
    ...
    if (svgIconCache.has(iconName)) {
        const cachedImg = svgIconCache.get(iconName);
        ctx.drawImage(cachedImg, x - size, y - size, size * 2, size * 2);
        return;
    }

    // Create and cache the SVG icon
    ...
    img.onload = () => {
        ...
        svgIconCache.set(iconName, img);
        render(); // Re-render to show the cached icon
    };
...
// Render function - called on-demand only (no continuous animation)
function render() {
    ctx.clearRect(0, 0, CONFIG.canvasSize, CONFIG.canvasSize);

    // Draw stars first (background layer) - static, no time parameter
    drawStars();
...
function makeCtrlBtn(text, onClick, isSpecial = false, ariaLabel = '') {
    ...
    btn.style.cssText = `
        ...
        border: 1px solid ${isSpecial ? '#333' : '#fff'};
        color: ${isSpecial ? '#666' : '#000'};
        font-size: 20px;
        font-weight: 300;
        cursor: pointer;
        box-shadow: ${isSpecial ? 'none' : '0 0 20px rgba(255, 255, 255, 0.3)'};
...
function updateEditCtrlButton() {
    if (selectedNode) {
        ...
        editCtrlBtn.style.background = '#fff';
        editCtrlBtn.style.borderColor = '#fff';
        editCtrlBtn.style.color = '#000';
        editCtrlBtn.style.boxShadow = '0 0 25px rgba(255, 255, 255, 0.6)';
    } else {
        editCtrlBtn.style.opacity = '0.4';
        editCtrlBtn.style.cursor = 'not-allowed';
