...
function initStars() {
    stars = [];
    const numStars = 70; // Increased from 25 to 60-80 range
    ...
    for (let i = 0; i < numStars; i++) {
        ...
        stars.push({
            x: x,
            y: y,
            radius: Math.random() * 2.0 + 1.0, // Bigger: 1.0-3.0
            alpha: Math.random() * 0.6 + 0.2   // Brighter: 0.2-0.8
        });
    }
    ...
    for (let i = 0; i < stars.length; i++) {
        for (let j = i + 1; j < stars.length; j++) {
            ...
            // Connect nearby stars within skill node distance
...
function drawStars(selectedNodeData = null, transitionFactor = 0) {
    ...
    if (selectedNodeData) {
        ...
        targetActivationRadius = importance * 80 + (importance > 0 ? 20 : 0);
        targetIntensity = 0.3 + (importance * 0.175);
    }
    ...
    constellationLines.forEach(line => {
        ...
        if (activationRadius > 0) {
            ...
            if (dist < activationRadius) {
                ...
                lineAlpha = Math.min(0.6, line.alpha + proximityFactor * activationIntensity * 0.5);
            }
        }
    ...
    stars.forEach(star => {
        ...
        if (activationRadius > 0) {
            ...
            if (dist < activationRadius) {
                const proximityFactor = 1 - (dist / activationRadius);
                starAlpha = Math.min(1.0, star.alpha + proximityFactor * activationIntensity);
                starRadius = star.radius * (1 + proximityFactor * 0.5 * transitionFactor);
            }
        }
...
function refreshLayout() {
    // Re-layout without reloading data
    if (treeRoot) {
        layoutTree(treeRoot);
        render();
    }
...
let selectionAnimationStart = 0;
let animationFrameId = null;
let currentStarBrightness = 0;      // Smooth star brightness (0-1)
let targetStarBrightness = 0;       // Target star brightness
let currentIconTransition = 0;      // Smooth icon color transition (0-1)
let targetIconTransition = 0;       // Target icon transition
...
function drawNode(node, timestamp = 0, colorTransition = 0) {
    const cx = CONFIG.canvasSize / 2;
    const cy = CONFIG.canvasSize / 2;
    const x = cx + node.x;
    const y = cy + node.y;
    ...
    // Smooth color transition between white (0) and black (1)
    const nodeTransition = isSelected ? colorTransition : (1 - colorTransition);
    const fillR = Math.round(255 * (1 - nodeTransition * (isSelected ? 1 : 0)));
    const fillG = fillR;
    const fillB = fillR;
    const nodeFill = isSelected ? `rgb(${255 - Math.round(255 * colorTransition)}, ${255 - Math.round(255 * colorTransition)}, ${255 - Math.round(255 * colorTransition)})` : CONFIG.nodeFill;

    if (isSelected) {
        ctx.shadowBlur = 35 * colorTransition;
        ctx.shadowColor = `rgba(255, 255, 255, ${0.8 * colorTransition})`;

        ctx.beginPath();
        ctx.arc(x, y, radius + 6, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(255, 255, 255, ${0.5 * colorTransition})`;
        ctx.lineWidth = 2;
        ctx.stroke();
    } else {
    ...
    // Node fill - smooth transition from white to black when selected
    ctx.beginPath();
    ctx.arc(x, y, radius, 0, Math.PI * 2);
    ctx.fillStyle = nodeFill;
    ctx.fill();

    // Add white border when selected (fades in)
    if (isSelected && colorTransition > 0) {
        ctx.strokeStyle = `rgba(255, 255, 255, ${colorTransition})`;
        ctx.lineWidth = 2;
        ctx.stroke();
    }
    ...
    // Calculate light wave effect (sweeps from bottom to top every 3 seconds)
    let lightWaveY = null;
    if (isSelected && timestamp > 0) {
        const elapsed = timestamp - selectionAnimationStart;
        const cycleTime = 3000; // 3 seconds per wave
        const cycleProgress = (elapsed % cycleTime) / cycleTime;
        // Wave position from bottom (-1) to top (1) of the icon
        lightWaveY = 1 - (cycleProgress * 2);
    }

    // Draw icon with color transition and light wave
    drawIcon(x, y, radius * 0.55, node.icon || 'default', isSelected, colorTransition, lightWaveY);
...
// Separate caches for normal (black) and inverted (white) icons
const svgIconCacheNormal = new Map();
const svgIconCacheInverted = new Map();

function drawIcon(x, y, size, iconName, isSelected = false, colorTransition = 0, lightWaveY = null) {
    // Check if this is a custom image path
    if (isCustomImagePath(iconName)) {
        ...
        if (state === 'loaded' && customImageCache.has(iconName)) {
            ...
            // Clip to circle for cropped fit
            ctx.beginPath();
            ctx.arc(x, y, size, 0, Math.PI * 2);
            ctx.clip();

            // Calculate crop to fit (cover mode - no stretching)
            ...
            // Apply smooth inversion transition for custom images
            if (colorTransition > 0) {
                ctx.filter = `invert(${colorTransition})`;
            }
            ctx.globalAlpha = 0.85 + (0.15 * colorTransition);
            ctx.drawImage(img, drawX, drawY, drawWidth, drawHeight);

            // Draw light wave overlay when selected
            if (lightWaveY !== null && colorTransition > 0.5) {
                ctx.globalCompositeOperation = 'lighter';
                const waveGradient = ctx.createLinearGradient(x, y + size, x, y - size);
                const wavePos = (1 - lightWaveY) / 2; // Convert -1..1 to 0..1
                const waveWidth = 0.15;
                waveGradient.addColorStop(Math.max(0, wavePos - waveWidth), 'rgba(255,255,255,0)');
                waveGradient.addColorStop(wavePos, `rgba(255,255,255,${0.3 * colorTransition})`);
                waveGradient.addColorStop(Math.min(1, wavePos + waveWidth), 'rgba(255,255,255,0)');
    ...
    // Default: Draw SVG icon from ICONS library
    const icon = ICONS[iconName] || ICONS.default;

    // For smooth transition, we draw both versions and blend
    ctx.save();

    // Draw black icon (fades out when selected)
    if (colorTransition < 1) {
        if (svgIconCacheNormal.has(iconName)) {
            ctx.globalAlpha = 1 - colorTransition;
            ctx.drawImage(svgIconCacheNormal.get(iconName), x - size, y - size, size * 2, size * 2);
        } else {
            // Create and cache black icon
            ...
            img.onload = () => { svgIconCacheNormal.set(iconName, img); render(); };
            if (img.complete) { svgIconCacheNormal.set(iconName, img); }
        }
    }

    // Draw white icon (fades in when selected)
    if (colorTransition > 0) {
        if (svgIconCacheInverted.has(iconName)) {
            ctx.globalAlpha = colorTransition;
            ctx.drawImage(svgIconCacheInverted.get(iconName), x - size, y - size, size * 2, size * 2);

            // Draw light wave overlay
            if (lightWaveY !== null && colorTransition > 0.5) {
                ctx.globalCompositeOperation = 'destination-in';
                const waveGradient = ctx.createLinearGradient(x, y + size, x, y - size);
                const wavePos = (1 - lightWaveY) / 2;
                const waveWidth = 0.2;
                waveGradient.addColorStop(0, 'rgba(255,255,255,1)');
                waveGradient.addColorStop(Math.max(0, wavePos - waveWidth), 'rgba(255,255,255,1)');
                waveGradient.addColorStop(wavePos, 'rgba(255,255,255,1)');
                waveGradient.addColorStop(Math.min(1, wavePos + waveWidth), 'rgba(255,255,255,0.7)');
                waveGradient.addColorStop(1, 'rgba(255,255,255,0.7)');
            }
        } else {
            // Create and cache white icon
            const svgNS = "http://www.w3.org/2000/svg";
            const svg = document.createElementNS(svgNS, "svg");
            svg.setAttribute("viewBox", icon.viewBox);
            svg.setAttribute("width", size * 2);
            ...
            const img = new Image();
            img.src = 'data:image/svg+xml;base64,' + btoa(svgData);
            img.onload = () => { svgIconCacheInverted.set(iconName, img); render(); };
            if (img.complete) { svgIconCacheInverted.set(iconName, img); }
        }
    }
...
// Render function - supports smooth transitions and animations
function render(timestamp) {
    const now = timestamp || performance.now();
    ctx.clearRect(0, 0, CONFIG.canvasSize, CONFIG.canvasSize);

    // Update smooth transitions
    const transitionSpeed = 0.08; // How fast transitions happen (0-1, higher = faster)

    // Star brightness transition
    targetStarBrightness = selectedNode ? 1 : 0;
    if (Math.abs(currentStarBrightness - targetStarBrightness) > 0.01) {
        currentStarBrightness += (targetStarBrightness - currentStarBrightness) * transitionSpeed;
    } else {
        currentStarBrightness = targetStarBrightness;
    }

    // Icon color transition
    targetIconTransition = selectedNode ? 1 : 0;
    if (Math.abs(currentIconTransition - targetIconTransition) > 0.01) {
        currentIconTransition += (targetIconTransition - currentIconTransition) * transitionSpeed;
    } else {
        currentIconTransition = targetIconTransition;
    }

    // Draw stars with smooth brightness transition
    drawStars(selectedNode, currentStarBrightness);
    ...
    // Draw all nodes with smooth color transition
    allNodes.forEach(node => drawNode(node, now, currentIconTransition));
}
...
// Animation loop for selected node
function animateSelection(timestamp) {
    if (!selectedNode) {
...
function openFile(node) {
    if (node.file) {
        ...
        const path = `${settings.skillFolder}/${node.name}.md`;
...
function makeCtrlBtn(text, onClick, isSpecial = false, ariaLabel = '') {
    ...
    btn.style.cssText = `
        ...
        font-weight: 300;
        cursor: pointer;
        box-shadow: ${isSpecial ? 'none' : '0 0 15px rgba(0, 0, 0, 0.5)'};
...
function updateEditCtrlButton() {
    if (selectedNode) {
        ...
        editCtrlBtn.style.boxShadow = '0 0 25px rgba(255, 255, 255, 0.4)';
    } else {
        editCtrlBtn.style.opacity = '0.4';
        editCtrlBtn.style.cursor = 'not-allowed';
