...
class EditableEmbedsMobilePlugin extends obsidian.Plugin {
	...
	async onload() {
		...
		if (!obsidian.Platform.isMobile && !this.settings.enableOnDesktop) {
	...
	setupEmbedsInContainer(container, sourcePath) {
	...
	makeEmbedEditable(embedEl, sourcePath) {
		...
		if (!linkedFile || !(linkedFile instanceof obsidian.TFile)) return;

		// Create edit overlay/button
		this.createEditInterface(embedEl, linkedFile);
	}

	createEditInterface(embedEl, file) {
		// Add edit button
	...
	async openEmbedEditor(embedEl, file) {
		...
		const debouncedSave = obsidian.debounce(
			async () => {
	...
	findHeadingSection(content, heading) {
	...
	findBlockSection(content, blockId) {
	...
	async saveEmbedContent(file, newContent, originalFullContent, sectionStart, sectionEnd, isSection) {
		try {
			let finalContent;
	...
	closeEmbedEditor(embedEl, editorContainer, originalContent) {
	...
	toggleAllEmbedEditing() {
		const activeView = this.app.workspace.getActiveViewOfType(obsidian.MarkdownView);
		if (!activeView) return;

		const container = activeView.containerEl;
		const embeds = container.querySelectorAll(
			".internal-embed[data-editable-embed], .markdown-embed[data-editable-embed]"
		);
		...
		embeds.forEach((embed) => {
			if (embed instanceof HTMLElement) {
				...
				if (editor) {
					// Close editor
					const originalContent = embed.querySelector(".markdown-embed-content");
					this.closeEmbedEditor(embed, editor, originalContent);
				} else {
					// Open editor
...
class EditableEmbedsMobileSettingTab extends obsidian.PluginSettingTab {
	...
	display() {
		...
		new obsidian.Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.showEditBorder).onChange(async (value) => {
		...
		new obsidian.Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.tapToEdit).onChange(async (value) => {
