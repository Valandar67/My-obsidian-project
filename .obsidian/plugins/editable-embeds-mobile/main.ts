...
interface AutoScrollEmbedsSettings {
	enabled: boolean;
	embedHeight: number;
	embedWidth: number;
}

const DEFAULT_SETTINGS: AutoScrollEmbedsSettings = {
	enabled: true,
	embedHeight: 400,
	embedWidth: 400,
};

export default class AutoScrollEmbedsPlugin extends Plugin {
	settings: AutoScrollEmbedsSettings;
...
class AutoScrollEmbedsSettingTab extends PluginSettingTab {
	plugin: AutoScrollEmbedsPlugin;

	constructor(app: App, plugin: AutoScrollEmbedsPlugin) {
		super(app, plugin);
		this.plugin = plugin;
	}
