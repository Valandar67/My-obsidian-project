...
### Home.md Embedding Mode (Optional)

Olen operates in two modes:

**Mode A: Full App (Default)** — Olen opens as a dedicated `ItemView` (as described above)

**Mode B: Embedded Widget** — For users who want to keep their `Home.md` as the entry point:
- Olen registers a custom codeblock processor `'''olen-dashboard`
- Users place this block anywhere in their existing `Home.md`
- Olen renders the full dashboard UI inside that codeblock using Obsidian's `MarkdownRenderer` or an iframe-like container
- The surrounding markdown (custom headers, other dataview blocks, personal notes) remains untouched
- Olen detects if it's running in embedded mode and adjusts:
  - Removes the Hero Background (inherits page background instead)
  - Collapses to a compact view by default
  - Adds an [Expand to Full View] button that opens Mode A
...
### Screen 5: Integration Choice

Present three options:

**"Keep My Home"** (Recommended for existing users)
> Olen embeds into your current `Home.md` as a live widget. 
> Your existing dataview blocks, notes, and structure stay exactly as they are.
> *Choose this if you have a customized homepage you don't want to lose.*

**"Fresh Start"** 
> Olen becomes your new dashboard. Your old `Home.md` is preserved as `Home-Archive.md`.
> *Choose this if you want the pure app experience.*

**"Side-by-Side"**
> Keep both. Olen opens via ribbon icon, your `Home.md` stays as your vault root.
> *Choose this if you're not ready to commit yet.*

[CTA varies based on selection]
...
### Home.md Compatibility Layer

**For existing Home.md users**, Olen provides a bridge, not a wall:

1. **Read-First Approach**: On first launch, Olen reads the existing `Home.md` structure:
   - Parses existing dataviewjs queries to extract current activity definitions
   - Detects `Home/Starts/` folder structure and imports activities automatically
   - Reads frontmatter properties from existing daily notes to populate historical data

2. **Bidirectional Sync** (Optional): If the user keeps their legacy dataview blocks:
   - Olen writes completions back to the properties that the old dataview queries expect
   - The old Home.md "still works" even while Olen runs—both show the same data
   - Users can gradually remove old dataview blocks as they trust Olen

3. **The `olen-embed` Codeblock**:

   ```markdown
   ## My Custom Section (User's existing content preserved)
   ```
   ```
   '''olen-dashboard
   view: compact
   sections: [oracle, day-map, boss-status]
   '''
   
   ```
   ## My Other Dataview Block (Unchanged)
   '''dataviewjs
   // user's existing code still runs
   '''
...
## New Section: XVII. Backwards Compatibility Philosophy

## XVII. The "No Abandonment" Principle

Olen must respect the **sunk cost** of a user's existing vault structure:

1. **No Forced Migration**: The user can use Olen for 6 months and still have their old Home.md work exactly as before
2. **No Proprietary Lock-in**: Olen still writes to standard markdown files (daily notes, session logs) so if Olen is uninstalled, the data is still readable
3. **Progressive Enhancement**: Start as a small embed, grow to full dashboard as the user opts-in
4. **Familiar Anchors**: Keep recognizable elements from the old Home.md (the corner decorations, the boss HP wording, the "Good evening" greeting) so it feels like an **evolution**, not a **replacement**
